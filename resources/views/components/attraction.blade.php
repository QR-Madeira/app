<div class='grid xl:grid-cols-5 grid-cols-2 p-4 w-full h-36'>
  ...
  <h1 class='xl:flex flex-row items-center text-xl hidden'>{{$attraction->title}}</h1>

  <div class="xl:block overflow-auto py-4 hidden">
    <p class='flex flex-row items-center text-xl max-h-28'>{{$attraction->description}}</p>
  </div>

  <div class='flex flex-col xl:flex-row xl:items-center items-end w-full justify-end space-x-2 xl:col-span-2'>
    ...
    @endif
    <button onclick="document.getElementById('{{$attraction->id}}').style.display = 'block';" class='xl:py-4 xl:px-6 py-1 px-2 rounded border-red-600 border-2 text-red-600 hover:text-white hover:bg-red-600'>@lang('Delete')</button>
  </div>
</div>
